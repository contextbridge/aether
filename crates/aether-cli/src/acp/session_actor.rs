...
use aether_core::events::{AgentCommand, AgentEvent, Command, ToolEvent, TurnEvent, TurnOutcome};
...
fn turn_stop_reason(message: &AgentEvent) -> Option<acp::StopReason> {
    match message {
        AgentEvent::Turn(TurnEvent::Ended { outcome: TurnOutcome::Cancelled }) => Some(acp::StopReason::Cancelled),
        AgentEvent::Turn(TurnEvent::Ended { .. }) => Some(acp::StopReason::EndTurn),
        _ => None,
    }
}
