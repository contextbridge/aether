...
use crate::events::{
    AgentCommand, AgentEvent, AgentObserver, Command, ContextUsage, LlmCallOutcome, LlmCallPurpose, TurnOutcome,
    UserCommand,
};
use crate::events::{ContextEvent, MessageEvent, ModelEvent, ToolEvent, TurnEvent};
...
impl Agent {
    ...
    async fn on_iteration_complete(&mut self, id: String, iteration: IterationState) {
        ...
        if has_content {
            ...
            self.emit(AgentEvent::Message(MessageEvent::Text {
                message_id: id.clone(),
            ...
            if !reasoning_summary_text.is_empty() {
                self.emit(AgentEvent::Message(MessageEvent::Thought {
                    message_id: id.clone(),
    ...
    async fn handle_tool_request_start(&mut self, id: String, name: String) {
        ...
        self.emit(AgentEvent::Tool(ToolEvent::Call { request, model_name: self.llm.display_name() })).await;
    }
    ...
    async fn handle_tool_request_arg(&mut self, id: String, chunk: String) {
        ...
        self.emit(AgentEvent::Tool(ToolEvent::CallUpdate {
            tool_call_id: id,
            chunk,
            model_name: self.llm.display_name(),
        }))
            ...
            ToolExecutionEvent::Complete { tool_id: _, result, result_meta } => match result {
                Ok(tool_result) => {
                    ...
                    if state.pending_tool_ids.remove(&tool_result.id) {
                        ...
                        self.emit(AgentEvent::Tool(ToolEvent::Result {
                            result: tool_result,
                            result_meta,
                            model_name: self.llm.display_name(),
                        }))
                ...
                Err(tool_error) => {
                    if state.pending_tool_ids.remove(&tool_error.id) {
                        ...
                        self.emit(AgentEvent::Tool(ToolEvent::Error {
                            error: tool_error,
                            model_name: self.llm.display_name(),
                        }))
                        .await;
    ...
    async fn finish_turn(&mut self, outcome: TurnOutcome) {
        if std::mem::take(&mut self.turn_active) {
            self.emit(AgentEvent::Turn(TurnEvent::Ended { outcome })).await;
        }
    }
