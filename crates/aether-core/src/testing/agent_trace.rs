...
impl AgentTrace {
    ...
    pub fn assert_names<const N: usize>(&self, expected: [&str; N]) {
        assert_eq!(map_event_names(&self.events), expected, "unexpected trace: {:?}", self.events);
    }

    pub fn assert_names_with_message<const N: usize>(&self, expected: [&str; N], message: &str) {
        assert_eq!(map_event_names(&self.events), expected, "{message}: {:?}", self.events);
    }
