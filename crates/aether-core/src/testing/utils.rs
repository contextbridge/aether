...
impl TestAgentBuilder {
    ...
    pub async fn run_with_context(self) -> Result<TestAgentResult, Box<dyn Error>> {
        ...
        // The command channel must stay open until a pending cancel is sent;
        // the agent only exits once the channel closes and its streams drain.
        let mut tx = self.cancel_when.is_some().then_some(tx);
        ...
        // The command channel is closed, so the agent task exits once its
        // streams drain; awaiting it guarantees observers saw every event.
