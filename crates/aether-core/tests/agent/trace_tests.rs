...
use llm::{ContentBlock, LlmError, LlmResponse};
...
async fn tool_call_turn_emits_full_trace() -> Result<(), Box<dyn Error>> {
    ...
    assert!(
        matches!(events.get(1), Some(AgentEvent::Turn(TurnEvent::Started { content })) if ContentBlock::join_text(content) == "3+5 = ?"),
        "turn start carries the user input: {events:?}"
    );
    ...
    for index in &call_starts {
        assert!(
            ...
            "chat calls start with attempt 0 and no delay: {:?}",
            events[*index]
        );
    }
...
async fn retried_call_traces_each_attempt() -> Result<(), Box<dyn Error>> {
    ...
    let retry_start = trace
        .events()
        .iter()
        .find(|e| matches!(e, AgentEvent::Turn(TurnEvent::LlmCallStarted { attempt: 1, .. })))
        .expect("retry attempt traced");
    assert!(
        matches!(retry_start, AgentEvent::Turn(TurnEvent::LlmCallStarted { delay_ms: Some(_), .. })),
        "retries carry their backoff delay: {retry_start:?}"
    );
...
#[tokio::test(start_paused = true)]
async fn cancel_during_call_traces_cancelled_call_and_turn() -> Result<(), Box<dyn Error>> {
    let attempts: Vec<Vec<Result<LlmResponse, LlmError>>> = vec![
        vec![Err(LlmError::ServerError { status: Some(503), message: "boom".into() })],
...
async fn compaction_call_is_traced_nested_in_the_chat_call() -> Result<(), Box<dyn Error>> {
    ...
    trace.assert_names_with_message(
        [
            "tool_definitions",
            "turn_started",
            "call_started:Chat:0",
        ...
        "compaction call should nest inside the chat call",
    );
