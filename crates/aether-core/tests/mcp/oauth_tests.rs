...
use mcp_utils::client::{
    DeferredToolRules, McpClientEvent, McpManager, McpServer, McpTransport, OAuthHandlerFactory, ToolExposure,
};
...
fn test_manager_with_home(with_oauth: bool) -> (tempfile::TempDir, McpManager) {
    ...
    let (event_tx, _) = mpsc::channel::<McpClientEvent>(50);
    let factory = if with_oauth { Some(fake_oauth_handler_factory()) } else { None };
    let manager = McpManager::new(event_tx, factory);
    (home, manager)
}
...
#[tokio::test]
async fn deferred_tools_with_failing_http_surface_failure() {
    let endpoint = FailingHttpEndpoint::bind().await;
    ...
    assert!(!statuses.iter().any(|s| s.name == "progressive-discovery"));
...
#[tokio::test]
async fn deferred_tools_support_partial_connections() {
    let endpoint = FailingHttpEndpoint::bind().await;
    let (_home, manager) = test_manager_with_home(false);
    let mut manager = manager.with_progressive_discovery_instructions(Arc::new(|servers| {
    ...
    let discovery_instructions =
        instructions.get("progressive-discovery").expect("Expected progressive discovery instructions");
    assert!(discovery_instructions.contains("working"), "Instructions should mention the connected server");
}
