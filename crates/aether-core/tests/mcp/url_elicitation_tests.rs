...
impl ServerHandler for UrlElicitationRequiredServer {
    ...
    async fn call_tool(
        ...
        Err(ErrorData::new(ErrorCode(-32042), "browser interaction required", Some(data)))
    }
}
...
impl ServerHandler for MalformedUrlElicitationRequiredServer {
    ...
    async fn call_tool(
        ...
        Err(ErrorData::new(ErrorCode(-32042), "browser interaction required", Some(self.data.clone())))
    }
}
...
async fn url_elicitation_required_accept_returns_retry_needed_error_without_url() {
    ...
    match captured.request {
        ...
        _ => panic!("expected URL elicitation request"),
    }
}
