...
use aether_sessions::testing::{
    agent_switched, assistant_chunk, assistant_text, tool_call, tool_error, tool_result, turn_ended, user_message,
};
...
fn empty_or_contentless_turns_do_not_add_messages() {
    ...
    assert_eq!(context_from_events(&[turn_ended()]).message_count(), 0);
}
...
fn complete_messages_are_reconstructed_but_streaming_chunks_are_not() {
    let context = context_from_events(&[
        assistant_chunk("partial", "partial"),
