...
use aether_core::events::{AgentEvent, AgentObserver, TurnEvent};
use aether_core::testing::{AddNumbersRequest, AgentTrace, DivideNumbersRequest, test_agent};
use aether_telemetry::{GenAiMetrics, OtelInstrumentation, OtelObserver};
...
async fn spans_form_a_turn_rooted_hierarchy() -> Result<(), Box<dyn Error>> {
    ...
    for chat in &chats {
        ...
        chat.assert_attr("gen_ai.system", "anthropic");
...
async fn failed_and_cancelled_calls_carry_error_attributes() -> Result<(), Box<dyn Error>> {
    ...
    let trace = test_agent()
        ...
        .cancel_when(|event| matches!(event, AgentEvent::Turn(TurnEvent::LlmCallStarted { attempt: 1, .. })))
        .user_text("go")
        .run_trace()
        .await?;
    ...
    assert_eq!(chats.len(), 2, "both call attempts produce spans: {:?}", spans.names());

    let failed = chats.iter().find(|span| span.attr("aether.llm.attempt") == Some(Value::from(0))).unwrap();
    ...
    let cancelled = chats.iter().find(|span| span.attr("aether.llm.attempt") == Some(Value::from(1))).unwrap();
    cancelled.assert_error_type("cancelled");
...
async fn compaction_call_is_tagged_and_parented_to_the_turn() -> Result<(), Box<dyn Error>> {
    ...
    let chats = spans.prefixed("chat").into_iter().filter(|span| span.attr("aether.llm.purpose").is_none());
    assert_eq!(chats.count(), 1, "the outer chat call still gets its own span");
    Ok(())
}
...
#[tokio::test]
async fn completed_calls_record_duration_and_token_metrics() -> Result<(), Box<dyn Error>> {
    let names = otel_test().redacting().observe_trace(&happy_tool_trace().await?).metric_names();
...
#[tokio::test]
async fn metric_attributes_stay_low_cardinality_even_when_capturing() -> Result<(), Box<dyn Error>> {
...
impl OtelTestBuilder {
    ...
    fn observe_trace(self, trace: &AgentTrace) -> ObservedTelemetry {
        let mut harness = self.build();
        harness.feed(trace.events());
        ObservedTelemetry { harness }
    }
...
impl ObservedTelemetry {
    fn spans(&self) -> Spans {
        self.harness.spans()
    }

    fn metric_names(&self) -> Vec<String> {
        self.harness.metric_names()
    }

    fn metric_attribute_keys(&self) -> BTreeSet<String> {
        self.harness.metric_attribute_keys()
    }
}
...
impl OtelHarness {
    fn new(capture_content: bool) -> Self {
        ...
        let metrics = GenAiMetrics::new(&meter_provider.meter("test"));
        let observer = OtelObserver::new(OtelInstrumentation {
            tracer: Some(tracer_provider.tracer("test")),
    ...
    fn metric_attribute_keys(&self) -> BTreeSet<String> {
        ...
        for resource in &self.metric_exporter.get_finished_metrics().expect("metrics exported") {
