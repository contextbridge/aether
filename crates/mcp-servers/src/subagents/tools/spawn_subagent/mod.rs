...
impl AgentExecutor {
    /// Create a new `AgentExecutor` with the given agent catalog, project
    /// root, and the cross-cutting dependencies handed to each spawned agent.
    ...
    async fn execute_single(&self, task_id: String, task: SubAgentTask) -> SubAgentResult {
        ...
        let result: Result<String, String> = async {
            ...
            if let Some(ref callback) = self.progress_callback {
                callback(&task_id, &agent_name, &AgentEvent::text("", "", false, ""));
            }
                ...
                match &message {
                    ...
                    AgentEvent::Turn(TurnEvent::Ended { outcome }) => {
                        match outcome {
                            ...
                            TurnOutcome::Failed { error } => error_message = Some(error.clone()),
                            TurnOutcome::Cancelled => {
                                was_cancelled = true;
                                error_message = Some("Processing cancelled".to_string());
                            }
                        }
                        break;
                    }
