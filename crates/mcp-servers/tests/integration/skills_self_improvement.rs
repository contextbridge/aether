...
async fn test_instructions_reference_list_skills_and_do_not_embed_catalog_entries() {
    let workspace = TestWorkspace::new()
        .file(
            ...
            skill().description("Agent skill").agent_authored(true).body("Content.").content(),
        )
        .file("skills/human-skill/SKILL.md", skill().description("Human skill").body("Content.").content());
...
async fn test_full_lifecycle() -> TestResult {
    let workspace = TestWorkspace::new().file(
        ...
        skill().description("Curated skill").body("# Curated\n\nHand-written skill.").content(),
    );
