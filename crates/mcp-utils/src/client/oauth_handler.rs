...
mod tests {
    ...
    #[tokio::test]
    async fn accepting_browser_prompt_keeps_waiting_for_callback() {
        ...
        assert!(authorize.await.unwrap().unwrap().contains("code=test-code&state=test-state"));
    }

    #[tokio::test]
    async fn configured_callback_port_uses_registered_redirect() {
    ...
    #[tokio::test]
    async fn configured_callback_port_fails_when_in_use() {
