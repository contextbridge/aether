...
impl ArtifactReviewSubmission {
    pub const DECISIONS: [&'static str; 2] = ["approved", "feedback"];

    pub fn fields(&self) -> impl Iterator<Item = (&'static str, &str)> {
        let (decision, feedback) = match self {
            Self::Approved => (Self::DECISIONS[0], None),
            Self::Feedback { feedback } => (Self::DECISIONS[1], Some(feedback.as_str())),
        };
        std::iter::once(("decision", decision)).chain(feedback.map(|feedback| ("feedback", feedback)))
    }
}
...
impl TryFrom<ReviewForm> for ArtifactReviewSubmission {
    ...
    fn try_from(form: ReviewForm) -> Result<Self, Self::Error> {
        match form.decision.as_str() {
            "approved" if form.feedback.is_empty() => Ok(Self::Approved),
            "approved" => Err("approval cannot contain feedback"),
            "feedback" => Ok(Self::Feedback { feedback: form.feedback }),
            _ => Err("decision must be approved or feedback"),
        }
    }
}
