...
fn selecting_non_oauth_server_is_noop() {
    ...
    assert!(ui.command_rx().try_recv().is_err(), "should not emit command for non-OAuth server");
}
...
fn authenticate_complete_updates_correct_entry() {
    ...
    ui.key(key(KeyCode::Down));
    ui.key(key(KeyCode::Enter));
    ui.key(key(KeyCode::Enter)); // Authenticate "a"
...
fn authenticate_failed_resets_to_needs_login() {
    ...
    ui.key(key(KeyCode::Down));
    ui.key(key(KeyCode::Enter));
    ui.key(key(KeyCode::Enter)); // Authenticate "x"
...
fn url_elicitation_complete_accepts_and_clears_settings_prompt() {
    ...
    LocalSet::new().block_on(&runtime, async {
        ...
        ui.acp_event(AcpEvent::McpNotification(McpNotification::UrlElicitationComplete(
...
fn server_status_updated_while_pane_open_refreshes() {
    ...
    ui.key(key(KeyCode::Tab));
    ui.key(key(KeyCode::Down));
    ui.key(key(KeyCode::Enter)); // Open MCP servers pane
...
fn esc_from_server_status_returns_to_menu() {
    ...
    ui.key(key(KeyCode::Tab));
    ui.key(key(KeyCode::Down));
    ui.key(key(KeyCode::Enter)); // Enter MCP Servers
    ui.key(key(KeyCode::Esc)); // Back to menu
...
fn server_status_navigation_skips_headers_and_spacers() {
    ...
    ui.key(key(KeyCode::Enter));
    assert!(ui.command_rx().try_recv().is_err(), "github is not OAuth, should be noop");

    // Move down once: should land on math (Proxied section, row index 4 - skipping Direct header)
    ui.key(key(KeyCode::Down));
    ui.key(key(KeyCode::Enter));
    assert!(ui.command_rx().try_recv().is_err(), "math is not OAuth, should be noop");
    ...
    ui.key(key(KeyCode::Enter));
    assert!(ui.command_rx().try_recv().is_err(), "math is not OAuth after wrap-up");

    // Move up again: should land back on github
    ui.key(key(KeyCode::Up));
    ui.key(key(KeyCode::Enter));
    assert!(ui.command_rx().try_recv().is_err(), "github is not OAuth after wrap-up");
}
...
fn proxied_oauth_server_sends_original_server_name() {
    ...
    ui.acp_event(mcp_notification(vec![proxied_oauth_server("linear", McpServerStatus::NeedsOAuth)]));
...
fn settings_builtin_opens_overlay_and_clears_composer() {
    ...
    assert!(ui.command_rx().try_recv().is_err());
...
fn settings_selecting_option_emits_config_option() {
    ...
    let cmd = ui.command_rx().try_recv().expect("expected set_config_option");
    match cmd {
...
fn settings_multi_select_toggle_and_confirm() {
    ...
    ui.type_text("/settings");
    ui.key(key(KeyCode::Tab));
    ui.key(key(KeyCode::Down)); // Skip Theme entry
    ui.key(key(KeyCode::Enter)); // Open model selector

    ui.key(key(KeyCode::Enter)); // Toggle first model
    ui.key(key(KeyCode::Esc)); // Confirm and close
...
fn model_selector_composed_space_does_not_toggle() {
    ...
    match ui.command_rx().try_recv().expect("plain space should select one model") {
        PromptCommand::SetConfigOption { config_id, value, .. } => {
            assert_eq!(config_id, "model");
...
fn config_option_update_failed_shows_in_transcript() {
    ...
    let items = ui.app_mut().drain_finalized();
...
fn new_session_clears_settings_overlay() {
    ...
    let _ = ui.command_rx().try_recv().ok();
}
...
fn settings_theme_picker_opens_and_shows_default() {
    ...
    ui.type_text("/settings");
    ui.key(key(KeyCode::Tab));
    ui.key(key(KeyCode::Enter)); // Open Theme picker (first entry)
...
fn settings_theme_selection_returns_to_menu() {
    ...
    ui.type_text("/settings");
    ui.key(key(KeyCode::Tab));
    ui.key(key(KeyCode::Enter)); // Open Theme picker
    ui.key(key(KeyCode::Enter)); // Confirm default
...
fn listed_themes_appear_in_the_theme_picker() {
    ...
    open_settings(&mut ui);
    ui.app_mut()
        .on_task_result(TaskResult::ThemesListed(vec!["dracula.tmTheme".to_string(), "kanagawa.tmTheme".to_string()]));
...
fn settings_menu_has_a_single_theme_row_after_themes_load() {
    let mut ui = TestUiBuilder::new().build();
    open_settings(&mut ui);
    ui.app_mut().on_task_result(TaskResult::ThemesListed(vec!["dracula.tmTheme".to_string()]));
...
fn rapid_theme_changes_settle_on_the_newest_choice() {
    ...
    open_settings(&mut ui);
    ui.app_mut()
        .on_task_result(TaskResult::ThemesListed(vec!["first.tmTheme".to_string(), "second.tmTheme".to_string()]));
...
fn theme_selection_keeps_overlay_open_and_refreshes_display() {
    ...
    ui.key(key(KeyCode::Tab));
    ui.key(key(KeyCode::Enter)); // Open Theme picker
    ui.key(key(KeyCode::Enter)); // Confirm default theme
...
fn model_selector_provider_heading_does_not_skip_rows() {
    ...
    ui.type_text("/settings");
    ui.key(key(KeyCode::Tab));
    ui.key(key(KeyCode::Down)); // Skip Theme
    ui.key(key(KeyCode::Enter)); // Open model selector
...
fn settle_theme_tasks_newest_first(ui: &mut TestUi) {
    loop {
        ...
        for settings in batch.into_iter().rev() {
            ui.app_mut().on_task_result(TaskResult::ThemeApplied { settings, theme: Theme::default(), error: None });
        }
    }
}
...
fn settings_picker_shows_unavailable_options_with_their_reason() {
    ...
    ui.key(key(KeyCode::Down));
    ui.key(key(KeyCode::Enter));
    assert!(ui.command_rx().try_recv().is_err(), "an unavailable option must not be selectable");
}
...
fn settings_picker_esc_returns_to_the_menu_without_changing_anything() {
    ...
    assert!(ui.command_rx().try_recv().is_err(), "backing out must not commit the focused value");
...
fn settings_picker_confirming_the_current_value_sends_nothing() {
    ...
    assert!(ui.command_rx().try_recv().is_err(), "re-picking the current value is not a change");
}
...
fn model_selector_toggling_twice_leaves_nothing_to_commit() {
    ...
    assert!(ui.command_rx().try_recv().is_err(), "toggling back to the original selection is not a change");
}
...
fn model_selector_preselects_the_current_value() {
    ...
    assert!(ui.command_rx().try_recv().is_err(), "closing without edits changes nothing");
}
...
fn model_selector_toggles_only_what_the_query_left_visible() {
    ...
    match ui.command_rx().try_recv().expect("expected the filtered model to be committed") {
