...
use super::state::{BottomBar, FullFileView, GitDiffLoadState, PatchCursor, PendingAction};
use super::task;
...
impl GitDiffScreen {
    ...
    pub(super) fn handle_event(&mut self, event: GitDiffEvent) -> Vec<GitReviewOutput> {
        ...
        match event {
            ...
            GitDiffEvent::FullFileLoaded { result, .. } => {
                match result {
                    Ok(content) if self.full_file.is_on() => self.full_file = FullFileView::Loaded(content),
                    Ok(_) => {}
                    Err(error) => {
                        self.full_file = FullFileView::Off;
                        self.bottom_bar = BottomBar::Error(error.to_string());
    ...
    fn on_browse_key(&mut self, key: KeyEvent) -> Vec<GitReviewOutput> {
    ...
    fn guarded(
        ...
        run: impl FnOnce(&mut Self) -> Vec<GitReviewOutput>,
    ) -> Vec<GitReviewOutput> {
    ...
    fn on_commit_editor_key(&mut self, key: KeyEvent) -> Vec<GitReviewOutput> {
        ...
        self.repo_operation(|request_id, repo_root| GitCommand::Commit { request_id, repo_root, message })
    }

    fn on_discard_confirm_key(&mut self, key: KeyEvent) -> Vec<GitReviewOutput> {
        if is_composed_char(key) {
            return Vec::new();
        }
        match key.code {
            KeyCode::Char('y' | 'Y') => {
                ...
                self.repo_operation(|request_id, repo_root| GitCommand::DiscardFile {
                    request_id,
                    repo_root,
                    path,
                    status,
    ...
    fn on_draft_key(&mut self, key: KeyEvent) -> Vec<GitReviewOutput> {
