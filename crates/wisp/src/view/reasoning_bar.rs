...
/// - `None` / `Default` => `default [···]` (all empty)
/// - `Low` => `low [■··]` (1 filled)
...
pub(crate) fn reasoning_bar(effort: Option<ReasoningEffort>, levels: &[ReasoningEffort], label_width: usize) -> String {
    ...
    format!("{label:>label_width$} {}", slot_bar(filled_slots(effort, levels), levels.iter().filter(|level| level.is_enabled()).count()))
}
...
/// Widest label these levels can show, including "default".
pub(crate) fn reasoning_label_width(levels: &[ReasoningEffort]) -> usize {
    levels.iter().map(|level| level.as_str().len()).chain(std::iter::once("default".len())).max().unwrap_or(0)
}
...
pub(crate) fn reasoning_color(effort: Option<ReasoningEffort>, levels: &[ReasoningEffort], theme: &Theme) -> Color {
    ...
    let total = levels.iter().filter(|level| level.is_enabled()).count();
    if total == 0 {
...
fn filled_slots(effort: Option<ReasoningEffort>, levels: &[ReasoningEffort]) -> usize {
    effort.filter(|effort| effort.is_enabled()).map_or(0, |effort| levels.iter().filter(|&&level| level.is_enabled() && level <= effort).count())
}
