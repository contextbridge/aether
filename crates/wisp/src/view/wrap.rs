...
pub fn wrap_line(line: Line<'static>, width: u16) -> Vec<Line<'static>> {
    ...
    loop {
        ...
        output.push(Line {
            spans: characters[row_start..row_end]
                .chunk_by(|left, right| left.style == right.style)
                .map(|run| {
                    Span::styled(run.iter().map(|character| character.character).collect::<String>(), run[0].style)
                })
                .collect(),
            style: line.style,
...
/// Byte offset where the longest suffix of `text` that fits within `max_width`
/// columns begins.
pub fn fit_suffix(text: &str, max_width: usize) -> usize {
    let mut width = 0;
...
/// Keeps the *last* `max_width` columns of `text`, prefixing an ellipsis when
/// content was dropped. The mirror of [`truncate_to_width`], for tickers that
/// scroll leftwards as new text arrives.
pub fn tail_to_width(text: &str, max_width: usize) -> String {
    ...
    let bytes = fit_suffix(text, max_width - ELLIPSIS_WIDTH);
...
mod tests {
    ...
    #[test]
    fn tail_to_width_keeps_the_end_of_the_text() {
        assert_eq!(tail_to_width("hello", 10), "hello");
        assert_eq!(tail_to_width("hello", 0), "");
        assert_eq!(tail_to_width("hello world", 7), "… world");
        assert!(tail_to_width("hello world", 7).width() <= 7);
    }

    #[test]
    fn tail_to_width_never_splits_a_wide_character() {
        let tail = tail_to_width("界界界界", 5);
        assert!(tail.width() <= 5, "{tail}");
        assert_eq!(tail, "…界界");
    }
