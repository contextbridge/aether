...
use agent_client_protocol::schema::v1::{self as acp, SessionConfigOption, SessionConfigSelectOption};
...
fn update_settings_options_never_renders_reasoning_row() {
    ...
    let updated_options = vec![SessionConfigOption::select(
        "model",
        "Model",
        "deepseek-chat",
        vec![
...
async fn tall_terminal_shows_more_picker_items() {
    ...
    let options = vec![SessionConfigOption::select("model", "Model", "model-0", many_models)];
