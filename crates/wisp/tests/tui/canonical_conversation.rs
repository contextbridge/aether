...
use wisp::conversation::{Conversation, ConversationContent, ItemState, MessageRole, ToolStatus};

fn assistant_chunk(conversation: &mut Conversation, text: &str) {
    conversation.append_message_chunk(MessageRole::Assistant, &acp::ContentChunk::new(text.into(), "reply"));
}
...
fn streaming_chunks_coalesce_and_revisions_advance() {
    ...
    assistant_chunk(&mut conversation, "Hel");
    let first_revision = conversation.items()[0].revision();
    assistant_chunk(&mut conversation, "lo");
...
fn assistant_content_is_sealed_by_the_end_of_the_turn() {
    ...
    assistant_chunk(&mut conversation, "answer");
    assert_eq!(conversation.items()[0].state(), ItemState::Open);
