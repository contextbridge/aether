...
pub(crate) async fn repo_identity(path: &Path) -> Option<String> {
...
/// Persistent record of workspaces the user has forked from or into.
#[derive(Clone)]
...
mod tests {
    ...
    fn register_and_list_roundtrip() {
        ...
        r.register(first.clone()).unwrap();
        r.register(second.clone()).unwrap();
        assert_eq!(r.list(), vec![first, second]);
    }
    ...
    fn register_ignores_already_registered_path() {
        ...
        r.register(original.clone()).unwrap();
        let renamed = ManagedWorkspace { name: "other-name".to_string(), ..original.clone() };
        r.register(renamed).unwrap();
        assert_eq!(r.list(), vec![original]);
    }
    ...
    fn list_hides_missing_paths_without_rewriting_file() {
        ...
        r.register(kept.clone()).unwrap();
        r.register(removed.clone()).unwrap();
        fs::remove_dir_all(&removed.path).unwrap();
        assert_eq!(r.list(), vec![kept.clone()]);
        fs::create_dir_all(&removed.path).unwrap();
        assert_eq!(r.list(), vec![kept, removed]);
    }
    ...
    fn register_leaves_no_temp_files() {
        let dir = TempDir::new().unwrap();
        let r = registry(&dir);
        r.register(workspace(&dir, "repo")).unwrap();
