...
mod tests {
    ...
    fn assert_tool_call_with_args_accepts_matching_json_arguments() {
        let report = report_with_messages(vec![tool_result("bash", r#"{"command":"pwd"}"#)]);
    ...
    fn assert_tool_call_with_args_surfaces_parse_failures() {
        let report = report_with_messages(vec![tool_result("bash", "not json")]);
    ...
    fn report_with_messages(messages: Vec<AgentMessage>) -> EvalReport {
