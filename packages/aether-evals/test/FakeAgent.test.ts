...
describe("FakeAgent", () => {
  it("success() ends with a done message", async () => {
    ...
    expect(result.messages.map(eventName)).toEqual([
      "message:text",
      "turn:ended",
    ]);
  });
  ...
  it("add supports observing each streamed message", async () => {
    ...
    expect(trace.messages.map(eventName)).toEqual(seen);
  });
  ...
  it("context_usage messages in the transcript flow into usage", async () => {
    ...
    expect(result.messages.map((event) => event.category)).toContain("context");
...
function eventName(event: AgentEvent): string {
  return `${event.category}:${(event.event as { type: string }).type}`;
}
