...
Any trailing args after plan flags are treated as an external submit command. When `submit_plan` is called, Aether appends the resolved absolute plan path as the final command argument. A zero exit status returns the command's stdout as `feedback` with `approved: false` (the reviewer decides); a non-zero status surfaces as a tool error including the command's stderr.
...
| ---------- | -------- | ------------------------------------------------------------------------ |
| `planName` | `string` | Stable plan identifier using only letters, numbers, dashes, and underscores; it cannot start or end with a dash or underscore. |
