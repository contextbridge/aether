...
`--project-root` anchors path resolution for the server. `--dir` is accepted as an alias; when omitted, the built-in in-memory server uses the workspace root. The standalone `mcp-servers-stdio --server subagents` binary discovers agents from `.aether/settings.json` in that directory (defaulting to the current directory). The built-in in-memory server instead uses the agent catalog Aether already resolved from settings at startup — it does not reload `.aether/settings.json` mid-session.
