...
`spawn_subagent` waits for the whole batch by default, so the parent receives all results before its next reasoning step. All children run concurrently, and the output contains one result per input task in input order. Rich child progress may be displayed while the call runs. Foreground batches are bounded by the parent agent's tool-call timeout (60 minutes by default), which cancels the call if the batch runs longer.
