...
`--dir` is accepted as an alias for `--project-root`. When omitted, the built-in in-memory server uses the workspace root. Sub-agents come from the agent registry resolved at startup: the embedded in-memory server never reloads `.aether/settings.json` from `--project-root` — explicit settings sources (such as SDK-supplied settings) stay authoritative at every delegation level. Only the standalone `mcp-servers-stdio --server subagents` binary discovers agents by loading `.aether/settings.json` from that directory (defaulting to the current directory).
