...
Filters are mutually exclusive: when several are provided, only one is applied, in the order `readyOnly`, `treeId`, `assignee`, `status`. With no filters, all tasks are returned.
