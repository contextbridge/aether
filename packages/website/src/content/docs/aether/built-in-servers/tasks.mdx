...
Only one filter is applied per call. If several are provided, the first match in this priority order wins: `readyOnly` → `treeId` → `assignee` → `status`. With no filters, all active tasks are returned.
...
The result includes the updated task, a `changes` list of the fields that were modified, and — when completing a task that others depended on — `newlyReady`: the tasks that just became ready to start.
