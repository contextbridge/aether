...
For persistent task storage, pass `--dir`. Aether stores JSONL task files under `<dir>/.aether-tasks/`: active trees live in `active/<root-task-id>.jsonl` and are moved to `completed/` when the root task completes.
