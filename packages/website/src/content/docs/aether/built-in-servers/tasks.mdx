...
For persistent task storage, pass `--dir`. Each task tree is stored as one JSONL file under `<dir>/.aether-tasks/active/`; archived trees are moved to `<dir>/.aether-tasks/completed/`.
