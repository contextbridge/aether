...
`task_update` returns the updated `task`, a `message`, and two fields that make updates easier to act on:

| Field         | Description                                                                       |
| ------------- | --------------------------------------------------------------------------------- |
| `changes`     | Names of the fields that were actually updated.                                   |
| `newlyReady`  | Tasks that became ready to start because this completion unblocked them.          |

`newlyReady` is only populated when a task is marked `completed` and other tasks depended on it.
