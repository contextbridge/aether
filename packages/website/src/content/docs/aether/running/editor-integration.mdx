...
Aether can run as an [Agent Client Protocol v2](https://agentclientprotocol.com/protocol/v2/migration) server. Editors that speak draft ACP v2 can start `aether acp`, create sessions, select Aether agents, stream tool progress, and handle MCP elicitation such as artifact review and permission requests.
...
| `--reasoning-effort` | `default \| disabled \| minimal \| low \| medium \| high \| xhigh \| max` | Initial reasoning effort for an explicit `--model`. Requires `--model`; mutually exclusive with `--agent`.                  |
...
Running plain `aether` starts onboarding if needed and then launches Aether's terminal UI. That UI is backed by the same ACP server path used by editors, so agent selection, MCP tools, elicitation, artifact review, and model configuration behave consistently across the terminal and editor integrations.
