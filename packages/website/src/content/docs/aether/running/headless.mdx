...
If prompt words are provided, Aether joins them with spaces (piped stdin is ignored). If no prompt is provided and stdin is not a TTY, Aether reads stdin, trims it, and uses that as the prompt.
...
| ------------------- | ------------------------ | ------------------------------------------------------------------------------------------------- |
| `[PROMPT]...`       | positional               | Prompt text. Reads stdin if omitted and stdin is not a TTY; a provided prompt takes precedence over stdin.                                       |
