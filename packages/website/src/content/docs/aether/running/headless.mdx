...
If prompt words are provided, Aether joins them with spaces and stdin is ignored. If no prompt is provided and stdin is not a TTY, Aether reads stdin, trims it, and uses that as the prompt.

```bash
# Prompt comes from the piped diff:
...
<Tabs>
  ...
  <TabItem label="json">
    Newline-delimited `AgentEvent` JSON. Each stdout line has a top-level `category` discriminator and a nested event. Most nested events carry their own `type` discriminator; `session_usage` events carry the usage payload directly.
...
| `--reasoning-effort` | `default \| disabled \| minimal \| low \| medium \| high \| xhigh \| max` | Reasoning effort for models that support extended thinking. `disabled` requires model and adapter support; unsupported disabling is rejected. |
