...
`reasoningEffort` is validated against the selected model's supported efforts when settings load: an unsupported value fails with an error listing the supported levels. For an alloyed model, the effort must be supported by every model in the alloy. Changing the model at runtime — for example from the terminal UI — clamps the current effort to the nearest level the new model supports.
