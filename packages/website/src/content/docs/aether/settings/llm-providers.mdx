...
| `requestModel`        | Overrides the model id sent in requests without changing the catalog identity used for capabilities, context window, and display. Honored by the OpenAI-compatible providers (DeepSeek, Fireworks AI, Microsoft Foundry, Moonshot, ZAI). |
| `inferenceProfileArn` | Bedrock application inference profile ARN to route requests through. Bedrock ignores `requestModel` when this is set.       |
