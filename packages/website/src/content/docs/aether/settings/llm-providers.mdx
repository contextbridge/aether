...
| Alloy        | `zai:glm-5.1,openai:gpt-5.5` | Round-robin between model specs on each LLM call.    |
...
| **Common models** | `codex:gpt-6-astra`, `codex:gpt-5.6-sol`, `codex:gpt-5.6-terra`, `codex:gpt-5.6-luna`, `codex:gpt-5.5`, `codex:gpt-5.4`, `codex:gpt-5.4-mini`, `codex:gpt-5.2` (272k context window) |
...
```json title=".aether/settings.json"
{
  ...
  "agents": [{ "model": "fireworks:accounts/fireworks/models/glm-5p3", "userInvocable": true }]
}
```
...
| Field                 | Description                                                                 |
| --------------------- | --------------------------------------------------------------------------- |
| `url`                 | Base URL for API requests. Honored by every built-in provider (Anthropic, OpenAI, OpenRouter, Gemini, Codex, Bedrock, the OpenAI-compatible providers, and the local servers). |
...
Providers expose only the effort levels they support. Requesting a level a model does not support is a settings validation error — Aether does not silently downgrade it. If you change models mid-session (for example through the terminal UI), Aether clamps the requested effort to the closest level the new model supports.

## Prompt caching and retries

Aether improves provider-side prompt caching automatically; no configuration is required:

- **Anthropic** — requests enable prompt caching and cache the system prompt.
- **Bedrock** — supported models get cache point markers.
- **OpenRouter / OpenAI-compatible** — Aether derives a per-conversation prompt cache key and sends it so providers can route repeated prefixes to cached input (Fireworks and Foundry included; DeepSeek, Moonshot, and ZAI requests omit it).

Transient failures (rate limits, 5xx responses, timeouts, and network/stream interruptions) are retried automatically with backoff.

## Alloying

An alloy spec is a comma-separated list of model specs. Every LLM call uses the next provider in the list, cycling round-robin — a single conversation turn that includes tool-call loops may therefore use several different models.
