...
Catalog IDs that use the Responses transport include `openai.gpt-5.6-luna`, `openai.gpt-5.6-sol`, `openai.gpt-5.6-terra`, `openai.gpt-5.5`, `openai.gpt-5.4`, `xai.grok-4.3`, `xai.grok-4.6`, `openai.gpt-oss-120b`, and `openai.gpt-oss-20b`. All other Bedrock models — including the dated `openai.gpt-oss-*-1:0` variants, `openai.gpt-oss-safeguard-*`, and region-prefixed IDs such as `global.openai.gpt-5.6-luna` — use the Converse API described above.
...
Aether validates `reasoningEffort` against the model when settings load: an effort the model does not support fails with an error listing the supported levels. When you switch models interactively (terminal UI or ACP), Aether snaps the current effort to the nearest supported level.

## Prompt caching

Aether automatically attaches cache routing keys to requests so consecutive turns reuse provider prompt caches:

- **OpenAI, Codex, and Bedrock Mantle** (Responses API transports) send a `prompt_cache_key` derived from the model, system prompt, and tool definitions — requests with the same prefix hash to the same key, so the provider routes them where the cache is already warm.
- **Microsoft Foundry** sends the same prefix-derived cache key.
- **Fireworks AI** sends a per-session affinity key so a conversation sticks to one cache location.

Cache keys are managed automatically; there is nothing to configure.
