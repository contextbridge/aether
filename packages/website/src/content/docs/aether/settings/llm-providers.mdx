...
|                  |                          |
| ---------------- | ------------------------ |
| **Credentials**  | `XIAOMI_API_KEY`         |
| **Model syntax** | `xiaomi:<model-id>`      |
| **Common model** | `xiaomi:mimo-v2.6-pro`   |
...
#### Models served by the Responses API

Some Bedrock models are not available through the Converse API. Aether routes these to OpenAI-compatible Responses endpoints instead. The model catalog decides the transport and endpoint automatically based on the model ID, so you configure them exactly like any other Bedrock model:
...
Catalog IDs that use the Responses transport include the `openai.gpt-5.6-*`, `openai.gpt-6-*`, `openai.gpt-5.5`, `openai.gpt-5.4`, `xai.grok-4.3`, and `openai.gpt-oss-*` families (for example, `bedrock:openai.gpt-5.5` or `bedrock:openai.gpt-oss-120b`). All other Bedrock models use the Converse API described above.

Requests go to one of two endpoints depending on the model ID:

- Global catalog IDs (for example `openai.gpt-5.5` or `openai.gpt-6-astra`) route to Bedrock Mantle at `bedrock-mantle.${AWS_REGION}.api.aws`.
- Cross-region inference profile IDs of the form `us.openai.gpt-6-*` (for example `us.openai.gpt-6-luna`) route through Bedrock Runtime's OpenAI-compatible Responses endpoint at `bedrock-runtime.${AWS_REGION}.amazonaws.com/openai/v1`.
...
| Value        | Meaning                                                             |
| ------------ | ------------------------------------------------------------------- |
| `"default"`  | Adapter default; does not promise reasoning is disabled             |
| `"disabled"` | Explicitly disable reasoning, only on supported models and adapters |
| `"minimal"`  | Minimal enabled thinking                                            |
...
Omitting `reasoningEffort` (or setting it to null) leaves the authored setting unspecified. Once overrides are resolved, it uses `default`. An explicit `"reasoningEffort": "default"` overrides an inherited selection. `"reasoningEffort": "disabled"` must be supported by every model in an alloy and by its adapter; otherwise the request fails without silently enabling reasoning. The application spelling `none` is not accepted.

Codex currently does not expose Disabled and retains Aether's Medium default. Generic toggle-only and Bedrock Converse disable encoders are not yet implemented, so their UI does not offer Disabled even when raw model metadata advertises a toggle.
