...
| `review`    | Markdown and HTML artifact review through elicitation                    | Optional `--root-dir <path>`                                                                                          |
...
<CardGrid>
  ...
  <LinkCard
    title="Review"
    description="Markdown & HTML artifact review and contextual feedback"
...
2. Reference a file source with `{ "type": "file", "path": "...", "deferTools": true }`, which defers every tool loaded from that file. Servers in that file that already declare their own `deferTools` rules keep those rules.
...
| `review__review_artifact` | Only the artifact review tool       |
