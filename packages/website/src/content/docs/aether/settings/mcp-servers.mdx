...
A server's `deferTools` value can be `false`, `true`, or a rule object. `false` (and omission) keeps every tool model-visible. `true` defers every tool. A rule object partitions MCP-local names with `include` and `exclude`; an empty `include` starts with every tool, and `exclude` wins on overlap.
