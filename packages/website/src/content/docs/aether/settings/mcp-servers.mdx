...
## Server-level tool filters

Each server entry accepts `tools: { "allow": [...], "deny": [...] }`. Patterns match tool names advertised by that server, before Aether adds its server namespace. Agent-level `tools` filters match the namespaced model names. Both must allow a tool for it to be listed or called; deferral does not bypass either filter. Empty allow means unrestricted, allow entries are OR alternatives, and deny wins. Name patterns support exact names and a trailing `*`; annotation predicates require matching annotations.

For an Aether gateway named `vm_tools`, server filters match `coding__read_file`, while agent filters match `vm_tools__coding__read_file`. See [remote workspace tools](/aether/running/remote-tools/) for paired deployment configuration, remote CLI composition, and security boundaries.
...
<Tabs>
  ...
  <TabItem label="HTTP">
    ...
    | `headers` | `object` | Custom HTTP headers, including `Authorization` and proxy routing/authentication headers. Values support variable expansion; invalid names/values and protocol-owned headers are rejected. |
    | `aetherGateway` | `boolean` | Enable Aether runtime identity and request policies for an [Aether remote tool server](/aether/running/remote-tools/). Default: `false`. |
...
| `review`    | Markdown artifact review through elicitation                             | Optional `--root-dir <path>`                                                                                         |
...
<CardGrid>
  ...
  <LinkCard
    title="Review"
    description="Markdown artifact review and contextual feedback"
    href="/aether/built-in-servers/review/"
  />
</CardGrid>

## MCP sources in settings

The `mcps` array in user or project settings can reference files, inline configs, optional files, or deferred files:
...
2. Reference a file source with `{ "type": "file", "path": "...", "deferTools": true }`, which defers every tool loaded from that file.
...
`deferTools`can be set to: `false`, `true`, or an object; it defaults to `false`. `false` means every tool will be eagerly loaded into context. `true` defers loading every tool for progressive discovery. An object allows controlling which tools within a single MCP server are deferred via `include` and `exclude` rules.
...
Invoke tools with exactly one JSON object supplied through `--json` or stdin. Tool calls time out after 600 seconds; pass `--timeout <seconds>` to change it. Because this is a real subprocess, it composes with ordinary Bash pipelines, redirects, `&&`, command substitution, and scripts:

```bash
aether mcp linear list_issues --json '{"state":"open"}' | jq .
printf '%s' '{"state":"open"}' | aether mcp linear list_issues
```

Only the private session socket and Aether executable directory are injected into Bash. Managed OAuth credentials stay inside Aether. Agent-level `tools.allow` and `tools.deny` filters govern both deferred discovery and execution, so denied tools remain unavailable.
...
| `review__review_artifact` | Only the Markdown review tool    |
...
**Planner tools with explicit artifact writes and review:**
  ...
  "agents": [
    {
      ...
      "mcps": [".aether/mcp.json"],
      "tools": {
        "allow": [{ "readOnly": true }, "coding__write_file", "coding__edit_file", "review__review_artifact", "skills__*"],
        "deny": ["coding__web_*"]
      }
    }
  ]
