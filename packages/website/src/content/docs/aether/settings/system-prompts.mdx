...
The onboarding wizard writes a base prompt file, `SYSTEM.md`, that agents inherit through the top-level `prompts` list:

```markdown title="SYSTEM.md"
# System Instructions
...
The batteries-included preset also writes an `aether-docs` skill under `.aether/skills/` (project scope) or `~/.aether/skills/` (user scope) to back that last guideline.

Agents inherit the top-level `prompts` list (which includes `SYSTEM.md`) by default. An agent can override it with its own `prompts` array — for example, the batteries-included wizard gives the Explore agent its own `AGENTS.md` (written under the `agents/` directory of the settings scope), so Explore replaces the top-level list and does _not_ receive `SYSTEM.md`.
