...
Aether assembles each agent's system prompt from the prompt sources in the agent's `prompts` field, followed by instructions from its configured MCP servers (`mcps`). There is no built-in system prompt, so you control every token in context.
...
The onboarding wizard writes a single base prompt file, `SYSTEM.md`, that agents inherit by default:
...
Agents inherit the top-level `prompts` list (which includes `SYSTEM.md`) by default. An agent can override it with its own `prompts` array — for example, the batteries-included wizard also writes `.aether/agents/codebase-explorer/AGENTS.md`, which is scoped to the Explore agent only.
