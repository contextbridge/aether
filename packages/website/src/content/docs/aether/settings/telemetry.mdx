...
Each agent turn produces an `invoke_agent` root span — named `invoke_agent <agent>` and carrying `gen_ai.agent.name` when the agent is known — so every LLM call beneath it is attributable to the agent that made it. Its child spans describe LLM calls (`chat <model>`) and tool executions (`execute_tool <name>`), including retries and terminal errors or cancellations.
