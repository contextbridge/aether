...
Each agent turn produces an `invoke_agent` root span (named `invoke_agent <agent>` when the agent has a name). Its child spans describe LLM calls (`chat <model>`) and tool executions (`execute_tool <name>`), including retries and terminal errors or cancellations.
