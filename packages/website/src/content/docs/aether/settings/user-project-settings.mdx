...
| `--provider <name>` | Provider to use, skipping the provider prompt. Supported: `anthropic`, `bedrock`, `codex`, `openai`, `openrouter`, and `zai` — the providers that ship curated model presets. Other providers are chosen interactively in the wizard. |
...
Settings can be passed directly to the CLI. `--settings-json` takes a JSON document inline, and `--settings-file` takes a path to one. Both use the same schema and bypass file loading:
