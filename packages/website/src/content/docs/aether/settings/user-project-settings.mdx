...
Project settings are merged over user settings:

- The top-level `agent` and `credentialsStore` from project settings win when set.
- A non-empty top-level `prompts` or `mcps` array in project settings replaces the user-level list.
- `providers` overrides merge per provider: fields set in project settings win, unset fields keep the user-level values.
- An agent defined in project settings with the same `name` as a user-level agent **replaces it entirely** — agents are never merged field-by-field. Agents without a name collision are combined.
...
Inline settings are not merged with file-based settings — user and project settings files are skipped entirely, so provide a complete settings document.
  ...
  "agents": [
    {
      ...
      "mcps": [
        { "type": "file", "path": "mcp.json", "deferTools": true },
        {
          "type": "file",
