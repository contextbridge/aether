...
The active scope is shown in the file-list header. In a repository that has no commits yet, the **Both** scope diffs everything against an empty tree so the initial work is still visible.
...
When queued review comments exist, actions that would clear them — switching scope, refreshing, staging or unstaging, committing, discarding, and closing the view — ask you to press the same key again to confirm.
