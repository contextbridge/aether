...
| `/move`     | Move changes to another workspace — pick an existing one or create a new workspace by name (when supported by the agent) |
...
- Image and audio files can be sent as media content blocks when both the agent and the selected model support them
