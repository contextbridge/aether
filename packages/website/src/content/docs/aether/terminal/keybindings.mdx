...
| `/move`     | Move uncommitted changes to another clone of the repository (when supported by the agent) |
...
`/move` opens a picker listing the other known clones of the current repository, plus a `Create new workspace…` entry. Selecting an existing clone moves your uncommitted changes there; selecting the create entry prompts for a name and clones a fresh sibling directory to move into. The session then continues in the target directory.
