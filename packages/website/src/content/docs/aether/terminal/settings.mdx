...
Themes are TextMate `.tmTheme` files. The default theme, named `Sage`, is built in. To use another theme, place its `.tmTheme` file in `~/.wisp/themes/` and reference it by filename in settings.
