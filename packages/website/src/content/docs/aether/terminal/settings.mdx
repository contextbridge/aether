...
- **Theme** — Switch between the built-in theme and your custom `.tmTheme` themes

Model, Mode, MCP servers, and Provider logins are advertised by the agent — the terminal UI discovers them at session start, and different agents may expose different options. Theme is a local UI preference stored in Wisp's settings file.
