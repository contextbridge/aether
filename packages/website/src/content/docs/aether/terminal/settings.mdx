...
| `gitRef`    | `"gitRef"`                        | Active git branch, or the short commit hash when detached.              |
...
Themes are TextMate `.tmTheme` files. Place them in `~/.wisp/themes/` and reference by filename in settings. A built-in theme, **Sage**, is used when no theme file is selected.
...
- **Theme** — Pick the built-in Sage theme or a `.tmTheme` file from `~/.wisp/themes/` (a local UI setting, stored in Wisp settings)
- **MCP servers** — View connected servers and their status
- **Provider logins** — Authenticate with providers that require it

These options are advertised by the agent — the terminal UI discovers them at session start. Different agents may expose different options. The theme entry is local to the terminal UI.
...
- **Provider logins** — Start the provider's login flow (such as OAuth). The flow itself is handled by the agent
