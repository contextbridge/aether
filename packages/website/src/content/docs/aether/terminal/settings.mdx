...
The top-level fields are `theme`, `statusLine`, and `contentPadding`. `contentPadding` sets the number of blank columns of padding around the conversation content (default `2`, minimum `2`). Status line segments are described below.
...
- **Model** — Override the model used by the currently active agent runtime. Each selected model exposes its own reasoning effort selector.
...
Reasoning effort is not a separate row — cycle it with `Tab` or set it inside the model selector.
