...
`AetherHeadlessOptions` mirrors most [headless CLI flags](/aether/running/headless/#flags) plus SDK process controls. The CLI's `mcpConfig`/`mcpConfigJson` and `output` options are intentionally not exposed — the SDK always emits newline-delimited JSON:
...
The agent delivers accounting over the ACP extension notification `_aether/session_usage`; the SDK decodes it into a domain-level `usage` message:
