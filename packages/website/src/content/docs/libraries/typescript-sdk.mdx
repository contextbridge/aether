...
Aether exposes accounting as a domain-level `usage` message in the prompt
stream (delivered over a private `_aether/session_usage` ACP extension
notification):
...
Overrides also accept `requestModel` (send a different model id in requests while keeping the catalog identity) and `inferenceProfileArn` (route Bedrock requests through an application inference profile). See [Provider overrides](/aether/settings/llm-providers/#provider-overrides) for details.
