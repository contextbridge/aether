...
| `reasoningEffort`     | `"default"`, `"disabled"`, `"minimal"`, `"low"`, `"medium"`, `"high"`, `"xhigh"`, or `"max"` when using a direct model. `"disabled"` requires a model and adapter that support disabling reasoning.                          |
