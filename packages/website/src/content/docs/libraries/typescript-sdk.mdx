...
| `reasoningEffort`     | `"default"`, `"disabled"`, `"minimal"`, `"low"`, `"medium"`, `"high"`, `"xhigh"`, or `"max"` when using a direct model. `"disabled"` must be supported by the model and adapter.                                              |
...
`AetherHeadlessOptions` mirrors most of the [headless CLI flags](/aether/running/headless/#flags) (`mcpConfig` is not available — headless runs take their MCP servers from settings), plus SDK process controls:
...
A spec on the top-level `mcps` is inherited by every agent that doesn't declare its own `mcps`. Put it on a single
