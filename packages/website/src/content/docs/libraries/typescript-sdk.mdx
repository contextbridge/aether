...
`AetherHeadlessOptions` mirrors the [headless CLI flags](/aether/running/headless/#flags) plus SDK process controls. It does not expose the CLI's `--mcp-config` / `--mcp-config-json` overrides — configure MCP sources through an inline `settings` object instead.
