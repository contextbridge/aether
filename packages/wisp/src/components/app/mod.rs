...
impl App {
    ...
    pub fn on_acp_event(&mut self, event: AcpEvent) -> EventOutcome {
        ...
        match event {
            ...
            AcpEvent::ForkOptionsLoaded(resp) => {
                let picker = WorkspacePicker::new(resp.options, self.working_dir.clone());
                self.conversation_screen.open_workspace_picker(picker);
            }
            AcpEvent::ForkOptionsFailed { error } => {
                self.conversation_screen.conversation.push_user_message(&format!("[wisp] {error}"));
            }
            AcpEvent::SessionForked(resp) => {
                let cwd = resp.cwd.clone();
                let git_ref = resp.git_ref.clone();
                let config_options = resp.config_options.clone();
                self.conversation_screen.close_modal();
                self.conversation_screen.reset_after_context_cleared();
                commands.push(RendererCommand::ClearScreen);
                let replay_updates = self.session_loading_buffer.take(&SessionId::new(resp.session_id.as_str()));
                for update in replay_updates {
                    self.on_session_update(&update);
                }
                self.update_config_options(&config_options);
                self.screen_router = ScreenRouter::new(cwd.clone());
                self.working_dir = cwd;
                self.workspace_status =
                    WorkspaceStatus::new(utils::paths::home_relative_path(&self.working_dir), git_ref);
            }
            AcpEvent::SessionForkFailed { session_id, error } => {
                self.session_loading_buffer.remove(&SessionId::new(session_id.as_str()));
                self.conversation_screen.fork_failed(&error);
            }
        }
                ...
                ConversationScreenMessage::OpenWorkspacePicker => {
                    if self.conversation_screen.is_waiting() {
                        self.conversation_screen
                            .conversation
                            .push_user_message("[wisp] Cannot fork while a prompt is in flight.");
                    } else if let Err(e) = self.prompt_handle.fork_options(&self.session_id) {
                        tracing::warn!("Failed to send fork_options: {e}");
                    }
                }
                ConversationScreenMessage::ForkWorkspace(destination) => {
                    self.session_loading_buffer.begin_load(self.session_id.clone());
                    if let Err(e) = self.prompt_handle.fork_session(&self.session_id, destination) {
                        self.session_loading_buffer.remove(&self.session_id);
                        tracing::warn!("Failed to send fork_session: {e}");
                    }
                }
