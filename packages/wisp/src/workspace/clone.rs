...
pub enum WorkspaceCloneError {
    #[error("invalid workspace name `{0}`: must be non-empty with no path separators")]
    InvalidName(String),
...
pub async fn cow_clone_dir(src: &Path, dest: &Path) -> Result<(), WorkspaceCloneError> {
    if !src.is_dir() {
    ...
    #[cfg(target_os = "linux")]
    {
        ...
        if stderr.contains("not supported") || stderr.contains("cross-device") {
    ...
    #[cfg(target_os = "macos")]
    {
        ...
        tokio::task::spawn_blocking(move || {
            ...
            // A directory clone is not atomic: clean up whatever partial tree was created.
...
pub fn validate_workspace_name(name: &str) -> Result<(), WorkspaceCloneError> {
    let valid = !name.trim().is_empty() && name != "." && name != ".." && !name.contains(['/', '\\', '\0']);
    if valid { Ok(()) } else { Err(WorkspaceCloneError::InvalidName(name.to_string())) }
}
