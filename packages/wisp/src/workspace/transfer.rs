...
pub enum WorkspaceTransferError {
...
///
/// Ignored files such as `.env` and `target/` are intentionally not transferred.
/// Staged-ness is flattened: changes arrive unstaged in the destination. Submodule
/// state transfer is out of scope.
pub async fn move_working_changes(src: &Path, dest: &Path) -> Result<TransferOutcome, WorkspaceTransferError> {
...
/// Clean tracked and untracked, non-ignored working tree changes from `src`.
///
/// Ignored files such as `.env` and `target/` are intentionally left untouched.
/// Staged-ness is flattened when restoring. Submodule state cleanup is out of scope.
pub async fn clean_working_changes(src: &Path) -> Result<(), WorkspaceTransferError> {
    let Some(src_root) = repo_root(src).await else {
        return Ok(());
    };
    if run_git(&src_root, &["rev-parse", "--verify", "HEAD"]).await.is_err() {
...
mod tests {
    ...
    async fn moves_tracked_modification_and_untracked_file() {
        ...
        let result = move_working_changes(&src, &dest).await.unwrap();

        assert_eq!(result, TransferOutcome::Moved);
        ...
        assert!(status(&src).is_empty(), "source status: {}", status(&src));
    }
    ...
    async fn dirty_destination_is_rejected_without_touching_trees() {
        ...
        assert!(matches!(result, Err(WorkspaceTransferError::DestinationDirty(_))), "got: {result:?}");
        assert!(src.join("new.txt").exists());
    ...
    async fn divergent_destination_rolls_back_copied_untracked_files() {
        ...
        assert!(matches!(result, Err(WorkspaceTransferError::PatchDoesNotApply(_))), "got: {result:?}");
    ...
    async fn unborn_head_is_rejected() {
        ...
        assert!(matches!(result, Err(WorkspaceTransferError::UnbornHead(_))), "got: {result:?}");
    }
