...
Aether assembles each agent's system prompt from authored prompt sources plus MCP server instructions. There is no built-in prompt file name to target; use the files and inline text you list in user settings (`$HOME/.aether/settings.json`) or project settings (`.aether/settings.json`).
  ...
  "agents": [
    {
      ...
      "prompts": [
        "PLANNER.md",
        { "type": "file", "path": "${WORKSPACE}/AGENTS.md", "optional": true },
        { "type": "glob", "pattern": "${WORKSPACE}/.aether/rules/*.md", "optional": true }
      ]
    }
  ]
}
...
In this user-level example, `PLANNER.md` is loaded from the Aether home, while the `AGENTS.md` and rules glob are loaded from whichever workspace is currently open.
