...
<Tabs>
  ...
  <TabItem label="pretty">
    Pretty-printed `AgentMessage` JSON for humans. Each emitted event is serialized with `serde_json::to_string_pretty`.
  ...
  <TabItem label="json">
    Newline-delimited `AgentMessage` JSON. Each stdout line is one compact object with a `type` discriminator such as `tool_call`, `text`, or `done`.

    ```bash
    aether headless --output json --events text,tool_call,tool_result,done \
      "List TODO comments"
    ```
...
Run declarative eval files in Docker sandboxes. Each eval runs the configured `agent.command`, which emits newline-delimited `AgentMessage` JSON, and aggregates pass/fail results.

```bash
aether eval evals/ --output json
```

See [Evals](/aether/running/evals/) for eval file examples, Docker requirements, and result handling.
