...
2. Content chunks — any mix of:
   ...
   - `LlmResponse::encrypted_reasoning(id, content)` — provider-encrypted reasoning payload
...
If an error occurs mid-stream, yield `Ok(LlmResponse::Error { message })` for a model-reported error, or `Err(LlmError)` for a provider/transport error.
